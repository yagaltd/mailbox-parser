...
Byte-parity is held by mirror implementations: `okf_escape_line` (Rust,
...
**Decision (2026-09-16):** no separate `--md` flag and no `src/okf.rs` library module —
...
function (`thread_to_okf_md`) only when a second consumer appears.
...
- `tests/fixtures/okf/` in mailbox-parser: fixture pairs `{name}.canonical.json` +
  ...
  tests/fixtures/okf/{name} --out OUT --format markdown --split-by thread
  [--attachments-dir OUT/attachments]` (same --out for the JSON run).
- mailbox-parser tests: `okf_fixture_goldens` in cli/src/main.rs —
  `render(canonical.json) == expected.md`
- MorphEditor tests: `tests/unit/okf-roundtrip.test.js` —
  `parseBlocks(expected.md).toMarkdown() === expected.md` (byte-stable round-trip);
  fixtures copied into MorphEditor `tests/fixtures/okf/`; drift breaks CI on
...
| T4 | Rust tests | S | **done** — inline `markdown_*` goldens + `okf_fixture_goldens` over tests/fixtures/okf/ |
| T5 | MorphEditor round-trip fixture test | S | **done** — `tests/unit/okf-roundtrip.test.js` + `tests/fixtures/okf/` in MorphEditor (4 pass) |
...
1. **List markers escaped instead of mapped** — body bullets/numbers render as
   ...
   **→ resolved (2026-09-16):** unordered (`-`/`*`/`+` + space) map to real
   ...
   Pinned in `sigil-escape` fixture + `okf_escape_line_grammar` test.
2. **`<mailto:addr>` wrappers leak verbatim** — strip to the bare address.
   **→ resolved (2026-09-16, hardened on re-run):** `okf_strip_mailto` handles
   ...
   Pinned in `refinements` fixture + `okf_strip_mailto_wrappers` test.
3. **Title keeps RE:/FW:/AW:/TR:/Ré: chains** — strip prefixes for the exported
   ...
   unaffected.
   **→ resolved (2026-09-16, hardened on re-run):** `okf_display_title`
...
4. **Attachment paths contain spaces** — invalid markdown URL syntax. Either
   ...
   **→ resolved (2026-09-16):** percent-encode the emitted path (URL-unreserved
   set + `/`, `okf_encode_path` — spaces/parens/UTF-8 → %XX); on-disk names
...
6. **Bare-URL autolinks not byte-stable** (found in the 2026-09-16 re-run):
   ...
   **→ resolved:** `okf_linkify_urls` mirrors their `^https?://[^\s<>\"]+`
...
Fixes driven by it (all in the renderer's shared `okf_body_line` pipeline):

7. **HTML→text link shapes** — the data layer emits `[imgUrl]<linkUrl>` (for
   `<a><img></a>`) and bare `[url]`; piecewise linkifying nested brackets.
   **→ resolved:** `okf_linkify_urls` recognizes `[url]<url2>` → `[url](url2)`,
